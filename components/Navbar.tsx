...
export function Navbar() {
    ...
    /* useEffect para detectar cuanto hemos scrolleado (< 80px) y asi poder
    colocar en modo trasnsparente el navbar */
    ...
    /* toggle menu con useCallback para mantener funcion estatica y fija, evitando renderizados y 
    instancias de funcion innecesarias */
    ...
    /* Animacion de los links del menu al aparecer en pantalla */
    ...
    /* 1. Resetear el estado cuando el menú se cierra */
    useEffect(() => {
        if (!menuOpen) {
            // Limpiamos la referencia para que la próxima vez empiece de cero
            prevImgRef.current = -1;

            // "Apagamos" visualmente todos los paneles para que no haya fantasmas
            // imgRefs.current.forEach((el) => {
            //     if (el) {
            //         gsap.set(el, {
            //             opacity: 0,
            //             scale: 0.92,
            //             overwrite: true // Detiene cualquier animación en curso
            //         });
            //     }
            // });
        }
    }, [menuOpen]); // Se dispara cada vez que abres o cierras el menú


    /* 2. Efecto de Crossfade (Animación de hover) */
    useEffect(() => {
        // Si el menú está cerrado, no ejecutamos lógica de hover
        if (!menuOpen) return;

        const prev = prevImgRef.current;

        // Evitamos animar si el elemento activo es el mismo que el anterior
        if (prev === activeImg) return;

        const prevEl = imgRefs.current[prev];
        const nextEl = imgRefs.current[activeImg];

        // ANIMACIÓN DE SALIDA: Si había una imagen previa, la ocultamos
        if (prevEl) {
            gsap.to(prevEl, {
                opacity: 0,
                scale: 0.92,
        ...
        // ANIMACIÓN DE ENTRADA: La nueva imagen siempre se muestra
        if (nextEl) {
            gsap.to(nextEl, {
                opacity: 1,
                scale: 1,
        ...
        // Actualizamos la "memoria" para el siguiente hover
        prevImgRef.current = activeImg;

    }, [activeImg, menuOpen]); // Añadimos menuOpen como dependencia para mayor seguridad
        ...
        <>
            ...
            <nav className={`fixed top-0 left-0 right-0 flex justify-between items-center px-6 py-7 md:px-12 transition-all  duration-300 ${scrolled ? 'bg-[rgba(10,10,10, 0.9)] backdrop-blur-[20px] border-b border-(--hex-border)' : ''}`} style={{ zIndex: 10000 }}>
                ...
                <div className='flex items-center gap-8'>
                    ...
                    <button onClick={toggleMenu} className="font-mono text-[11px] font-bold tracking-[0.25em] uppercase text-foreground bg-transparent border-none flex items-center gap-3">
